...
.factory('SOMService', function SOMServiceWW($q, WebWorkerService, _) {
	...
	SOM = function() {
			...
			if(withworker)  {
				...
				workerPromise.then(
					...
					function(result){
						...
						deferred.resolve({'som': result.payload.som});
					}, 
				...
				set_training_data({'som': som, 'data': data, 'sampleids': sampleids});
		...
		function get_best_matching_units_ww(input, output) {
			...
			var somobj = {M: input.M, N: input.N};
		...
		function calculate_permutations(input, output) {
			...
			for(var it=0;it<Nperm;it++) {
				...
				for(var i=0; i<input.rows*input.cols; i++) {
					average_values[i] = 0;
					samples_in_cell[i] = 0;
				}
		...
		function calculate_component_plane(som, sampleids, data_column, variable_name) {
			...
			var workers = [];
			for(var w=0;w<som.num_workers;w++) {
			...
			for(var w=0;w<som.num_workers;w++) {
				...
				obj = { rows: som.rows, 
						...
						som.codebook, 
						values : values, 
						nperm: Nperm/som.num_workers};	
				workerPromises[w] = workers[w].run(obj);
				
			}

			workerPromises[0].then(function(){}, function(err){ console.log(['error',err])}, function(progress) {
			...
			function calculate_plane_summary(results) {
				...
				for(var w=1;w<som.num_workers;w++) {
		...
		*/
		...
		/*		som.neighdist = 2;
				...
				deferred.notify(100);
				deferred.resolve({'plane': plane_object});
		...
		function run_SOM_epoch_ww(som) {
			...
			var workerPromises = [];
			for(var w=0;w<som.num_workers;w++) {
				obj = {M: som.M, N: som.N, samples: som.sample_slices[w], units: som.codebook};	
			...
			function iterate(results) {
				...
				if(new_neighbourhood_radius > som.end_neigh_dist) {
					...
					for(var w=0;w<som.num_workers;w++) {
						obj = {M: som.M, N: som.N, units: som.codebook};	
						workerPromises[w] = workers[w].run(obj);
					}
					...
					$q.all(workerPromises).then( iterate, function errFn(reasons) {
					  ...
					  deferred.notify(100);
    				  deferred.resolve({message: "SOM is trained"});
		...
		function set_training_data(input,output) {
			...
			var slice_length = Math.ceil(som.N / som.num_workers);

			for(var s = 0;s<som.num_workers; s++) {
